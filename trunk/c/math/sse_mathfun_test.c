...
float tbsinf( float xx ); 
float tbsin( float xx ); 
...
DECL_SCALAR_FN_BENCH(tbsinf);
DECL_SCALAR_FN_BENCH(tbsin);
...
#endif
	run_bench("tablized sinf ", bench_tbsinf);
	run_bench("tablized sin  ", bench_tbsinf);
	return err;
}
...
void test1()
{
	...
	double dsttbsin = tbsin(src);
	double dsttbsinf = tbsinf(src);
	printf("%32f \n", SQRT2_2);
	printf("%32f \n", dstsinf );
	printf("%32f \n", dsttbsin );
	printf("%32f \n", dsttbsinf );
